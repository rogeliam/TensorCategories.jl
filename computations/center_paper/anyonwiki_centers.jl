...
for (i,cat) in pairs(codes) 
    ...
    # Load the results
    t5 = @elapsed Z4 = load_fusion_category(joinpath(dir, "center_$(cat[1])_$(cat[2])_$(cat[3])_$(cat[4])_$(cat[5])_$(cat[6])_$(cat[7])"))
    ...
    println("Split in $t2 seconds")
    println("Skeletonized in $t3 seconds")
    println("Quick pentagon check $(randomized_pentagon_axiom(Z2, 3) ? "passed" : "failed")")
    println("Saved in $t4 seconds")
    println("Loaded in $t5 seconds")
    println("Quick check of loaded category $(randomized_pentagon_axiom(Z4, 3) ? "passed" : "failed")")
    ...
    write(log, "Split in $t2 seconds\n")
    write(log, "Skeletonized in $t3 seconds\n")
    write(log, "Quick pentagon check $(randomized_pentagon_axiom(Z2, 3) ? "passed" : "failed")\n")
    write(log, "Saved in $t4 seconds\n")
    write(log, "Loaded in $t5 seconds\n")
    write(log, "Quick check of loaded category $(randomized_pentagon_axiom(Z4, 3) ? "passed" : "failed")\n\n")
    flush(log)
end
