#=----------------------------------------------------------
    ...
    Parallel version with named command line arguments.
    ...
    Examples:
        ...
        julia anyonwiki_centers.jl --workers 16 --threads 1

        # All codes, 1 worker, 16 Julia threads
        julia anyonwiki_centers.jl --workers 1 --threads 16

        # Only global indices 25 through 40 in the deterministic filtered list
        julia anyonwiki_centers.jl --workers 4 --threads 4 --first 25 --last 40
...
function usage()
    println("""
Usage:
    julia anyonwiki_centers.jl [OPTIONS]
